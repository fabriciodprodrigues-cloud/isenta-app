          ...
          <CardBody className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Badge size="sm" variant="default">
                  {vehicle.category === 'oficial' && 'Oficial'}
                  {vehicle.category === 'ambulancia' && 'Ambulância'}
                  {vehicle.category === 'bombeiro' && 'Bombeiro'}
                  {vehicle.category === 'outro' && 'Outro'}
                </Badge>
              </div>
            </div>
          </CardBody>
          ...
          <CardBody className="space-y-4">
            <div>
              ...
              >
                ...
                {vehicle.status === 'aguardando' && 'Aguardando Resposta'}
              ...
              {vehicle.registrations.map((reg: any) => (
                <div key={reg.id} className="flex items-center justify-between p-3 rounded-lg border border-white/8 bg-ink-700/50">
                  ...
                  <Badge
                    ...
                    size="sm"
                  >
                    {reg.status === 'rascunho' && 'Rascunho'}
                    {reg.status === 'enviado' && 'Enviado'}
                    {reg.status === 'aguardando_resposta' && 'Aguardando'}
                    {reg.status === 'aprovado' && 'Aprovado'}
                    {reg.status === 'recusado' && 'Recusado'}
                  </Badge>
                </div>
              ))}
            </div>

import { NextRequest, NextResponse } from 'next/server';
import { put } from '@vercel/blob';
import { auth } from '@/lib/auth';
import { prisma } from '@/lib/prisma';
import { podeAcessarArtespDocumento } from '@/lib/artesp-acesso';

export const dynamic = 'force-dynamic';

const TAMANHO_MAXIMO = 10 * 1024 * 1024; // 10MB

/** Recebe o PDF assinado via gov.br/ICP-Brasil e marca o documento como assinado. */
export async function POST(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  const session = await auth();
  if (!session?.user) {
    return NextResponse.json({ error: 'Não autorizado' }, { status: 401 });
  }

  const permissao = await podeAcessarArtespDocumento(session.user as any, params.id);
  if (!permissao.ok) {
    return NextResponse.json({ error: permissao.erro }, { status: permissao.status });
  }

  const form = await request.formData();
  const arquivo = form.get('file');

  if (!(arquivo instanceof File)) {
    return NextResponse.json({ error: 'Arquivo é obrigatório' }, { status: 400 });
  }

  if (arquivo.size > TAMANHO_MAXIMO) {
    return NextResponse.json(
      { error: `Arquivo muito grande. Máximo 10MB (enviado: ${(arquivo.size / 1024 / 1024).toFixed(2)}MB)` },
      { status: 400 }
    );
  }

  if (arquivo.type !== 'application/pdf') {
    return NextResponse.json({ error: 'Envie o PDF assinado.' }, { status: 400 });
  }

  const caminho = `artesp/${permissao.documento.artespCadastroId}/${permissao.documento.tipo}-assinado.pdf`;
  const blob = await put(caminho, arquivo, {
    access: 'private',
    addRandomSuffix: true,
    contentType: 'application/pdf',
  });

  const atualizado = await prisma.artespDocumento.update({
    where: { id: params.id },
    data: { urlAssinado: blob.pathname, status: 'assinado', assinadoEm: new Date() },
  });

  return NextResponse.json(atualizado);
}
